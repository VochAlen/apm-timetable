...
// Helper funkcija za filtriranje letova (jednodnevni letovi se preskaču)
function filterFlights(flights) {
  return flights.filter(flight => {
    ...
    // Preskoči jednodnevne letove (start = end), ali zadrži year-round
...
export function generateTimetablePDF(groupedData, type, seasonLabel, periodLabel, airportConfig = {}) {
  ...
  // Filtriraj letove za svaku destinaciju pre prikazivanja
  const filteredGroupedData = groupedData.map(group => ({
    ...group,
    flights: filterFlights(group.flights)
  })).filter(group => group.flights.length > 0); // Ukloni destinacije bez letova nakon filtriranja

  // Ako nema letova nakon filtriranja, prikaži poruku
  if (filteredGroupedData.length === 0) {
    doc.setFont("helvetica", "normal");
    doc.setFontSize(12);
    doc.setTextColor(...COLORS.dark);
    doc.text("No seasonal flights found (all flights are one-day only)", PAGE_W / 2, PAGE_H / 2, { align: "center" });
    return doc;
  }
  ...
  for (let gi = 0; gi < filteredGroupedData.length; gi++) {
    const group = filteredGroupedData[gi];
    ...
    // Broj letova - mala oznaka (nakon filtriranja)
